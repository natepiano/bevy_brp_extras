...
fn discover_component_format_with_error_info(
    ...
    match &type_info {
        TypeInfo::Struct(struct_info) => {
            match generate_spawn_format_for_struct_with_error_handling(
                struct_info,
                world,
                debug_info,
            ) {
                Ok(spawn_format) => {
                    let mutation_info = generate_mutation_info_for_struct_with_debug(
                        struct_info,
                        world,
                        debug_info,
                    );
    ...
    if let Some(first_variant) = enum_info.iter().next() {
        ...
        match first_variant {
            ...
            VariantInfo::Tuple(v) => {
                    ...
                    if let Some(field) = v.field_at(0) {
                        ...
                        match discover_type_format_recursive_with_error_handling(
                            ...
                            Ok(field_value) => {
                                ...
                                return Ok(json!({ v.name(): field_value }));
                            }
                            Err(error_info) => return Err(error_info),
                        }
                    }
                ...
                for (index, field) in v.iter().enumerate() {
                    match discover_type_format_recursive_with_error_handling(
                        world,
                        field.type_path(),
                        debug_info,
                ...
                if unconstructable_fields.is_empty() {
                    Ok(json!({ v.name(): fields }))
                } else {
                    let mut error_info = serde_json::Map::new();
                    error_info.insert(
                        "reason".to_string(),
                        json!("Enum variant contains unconstructable fields"),
                    );

                    let field_details: Vec<String> = unconstructable_fields.iter()
                        .map(|(index, type_path, _)| format!("Field {index} has type {type_path} which cannot be represented in JSON"))
                        .collect();
                    error_info.insert("details".to_string(), json!(field_details.join("; ")));
            ...
            VariantInfo::Struct(v) => {
                ...
                if unconstructable_fields.is_empty() {
                    ...
                    let field_details: Vec<String> = unconstructable_fields.iter()
                        .map(|(name, type_path, _)| format!("Field '{name}' has type {type_path} which cannot be represented in JSON"))
                        .collect();
                    error_info.insert("details".to_string(), json!(field_details.join("; ")));
        ...
        error_info.insert(
            "details".to_string(),
            json!(format!(
                "Enum '{}' has no variants to use as an example",
                enum_info.type_path()
            )),
        );
        Err(error_info)
